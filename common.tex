...
\__wisper_declare_feature:nnn { supershortnotecite }
  ...
  {
    % TODO: read ahead for full-stop, commas, semicolon etc instead of 
    % manually specifying the symbol.
    ...
    \NewDocumentCommand \citep { D<>{} o o m }
      {
        % new command with two arguments: optional (#1) and mandatory (#2)
        % set value of \spc variable to the width of #1 argument
        \settowidth { \spc } {#1}
        % subtract from \spc about two (1.8) of its values making its magnitude negative
