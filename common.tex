...
\cs_new:Npn \__wisper_load_with_optional_arg:NNN #1#2#3
  {
    \clist_new:N \l__wisper_optional_arg_clist
    \__wisper_build_clist_from_prop:NN #2 \l__wisper_optional_arg_clist
    \__wisper_if_debug:T
      {
        \clist_show:N \l__wisper_optional_arg_clist
        \str_show:N #3
      }
    \exp_args:Noo \__wisper_load_with_optional_arg_aux:Nnn #1
      {
        \clist_use:Nn \l__wisper_optional_arg_clist { ,~ }
      }
      {
        \str_use:N #3
      }
  }
\cs_generate_variant:Nn \__wisper_load_with_optional_arg:NNN { NNn }
...
\cs_new:Npn \__wisper_declare_package:n #1
  {
    \prop:new:N \g_wisper_package_options_#1_prop
    \__wisper_define_keys_doc:n
      {
        packages/#1/enable .bool_set:n = \g__wisper_package_load_#1,
        packages/#1/enable .initial = {true|false},
        packages/#1/options / unknown .code:n =
          {
            \prop_put:NVn \g__wisper_package_options_#1_prop
              \l_keys_key_tl
              {#1}
          }
        packages/#1/conflicts / classes .tl_set:N = \g__wisper_package_#1_conflicting_classes,
        packages/#1/conflicts / packages .tl_set:N = \g__wisper_package_#1_conflicting_packages,
        packages/#1/conflicts / classes .initial:n = {koma},
        ...
        packages/#1/deactivate .bool_set:n = \g__wisper_package_deactivated_#1,
      }
  }
...
\cs_new:Npn \__wisper_enable_package:n #1
  {
    \__wisper_define_keys_doc:n
      {
        packages/#1/enable = true,
      }
  }

\cs_new:Npn \__wisper_set_package_options:nn #1#2
  {
    \__wisper_set_keys_doc:nn { packages/#1/options } {#2}
  }
...
\cs_new:Npn \__wisper_load_package:n #1
  {
    \__wisper_load_with_optional_arg:NNn
      \usepackage
      \g__wisper_package_options_#1_prop
      {#1}
  }
