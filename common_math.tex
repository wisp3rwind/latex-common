...
\NewDocumentCommand \Order { s m }
  {
    \__wisper_operator_order: \__wisper_parenthesize:nn {#1} {#2}
  }
\ExplSyntaxOff
...
\NewDocumentCommand \__wisper_integrate:nw { >{ \SplitArgument{2}{,} } r() m }
  { \__wisper_integrate_aux:nnnnn #1 { \__wisper_parenthesize_aux:n } {#2} }
...
\cs_new:Npn \__wisper_difsymbol:NNnnn #1#2#3#4#5
  {
    % #1 - the symbol
    % #2 - surrounding
    ...
    \IfNoValueTF {#5}
      {
        ...
        #1 \IfValueT{#4}{ \c_math_superscript_token {#4} }
      }
      {
        \__wisper_dif_spacing:nn {#3}
          {
            #1
            \IfValueT {#4} { \c_math_superscript_token {#4} }
            #2 {#5}
          }
      }
  }
\cs_new:Npn \__wisper_declare_difsymbol:nN #1#2
  {
    ...
    \exp_args:Nc \NewDocumentCommand { __wisper_ #1 _normal:nnn } { m m g }
      { \__wisper_difsymbol:NNnnn #2 {} {##1} {##2} {##3} }
    \exp_args:Nc \NewDocumentCommand { __wisper_ #1 _long:nnw } { m m r() }
      { \__wisper_difsymbol:NNnnn #2 { \__wisper_parenthesize_aux:n } {##1} {##2} {##3} }
    \exp_args:Nc \NewDocumentCommand { #1 } { s e{^} }
      % Full syntax:
      % \dif^3 {x}
      % \dif {x^3}
      % \dif^3 (x)
